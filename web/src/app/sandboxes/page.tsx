    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-200 font-sans p-4 md:p-8">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div>
          ...
          <p className="text-sm text-neutral-500 mt-1">Isolated execution environments across your Mac fleet</p>
        </div>
        ...
        <section className="bg-neutral-900 border border-neutral-800 rounded-lg overflow-hidden">
          <table className="w-full text-left text-sm">
            <thead className="bg-neutral-950 border-b border-neutral-800">
              <tr>
                <th className="px-6 py-3 text-neutral-400 font-medium">Sandbox ID</th>
            ...
            <tbody className="divide-y divide-neutral-800">
              {loading ? (
                <tr><td colSpan={7} className="px-6 py-8 text-center text-neutral-500">Loading sandboxes...</td></tr>
              ) : sandboxes.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-6 py-12 text-center">
                    <div className="text-neutral-500">No sandboxes created yet</div>
                    <div className="text-xs text-neutral-600 mt-1">
                      Run a command with <code className="text-emerald-400">--sandbox &lt;id&gt;</code> to create one
                    </div>
                  </td>
                </tr>
              ) : (
                sandboxes.map(s => (
                  <tr key={s.id} className="hover:bg-neutral-800/20">
                    <td className="px-6 py-4 font-mono text-xs text-neutral-300">{s.id}</td>
                    <td className="px-6 py-4 font-mono text-xs text-neutral-400">{s.machine_id}</td>
                    <td className="px-6 py-4 text-neutral-300">{s.name || '—'}</td>
                    <td className="px-6 py-4 font-mono text-xs text-neutral-500 max-w-[200px] truncate">{s.dir_path}</td>
                    ...
                    <td className="px-6 py-4 text-xs text-neutral-500">{formatDate(s.created_at)}</td>
